...
def dlm3u8(url,fname):
        ...
        # -reconnect_at_eof 1 を削除し、終了時の無限ループを防止。
        # 音声形式がAACであるため、mp3コンテナに保存する場合はエンコードが必要。-c copy を削除してエンコードさせる。
        command = f'ffmpeg -y -reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 300 -i "{url}" "{fname}"'
